% write_simple_inversion.m: A template for writing your own Simple Inversion
...
siv.phi = zeros(M, N); % optional* | Deflection potential in the object plane, the gradient of which gives the deflection angles. Omit only if the solution is not representable by a potential -- in which case specifying the deflection angles is required. | meters
% siv.defl_ax1 = zeros(M,N); % optional* | Deflection angle along axis-1 (= grad phi, axis 1 component). Values can be positive (for deflection in +ax1 direction) or negative (for deflection in -ax1 direction). | rad
% siv.defl_ax2 = zeros(M,N); % optional* | Deflection angle along axis-2 (= grad phi, axis 2 component). Values can be positive (for deflection in +ax1 direction) or negative (for deflection in -ax1 direction). | rad

% disp(siv.object_type) % already-set | Specification of the HDF5 object type | "inversion" (always this value)
% disp(siv.inversion_type) % already-set | Specification of the inversion sub-type | "simple" (always this value)
% disp(siv.pradformat_version) % already-set | HDF5 pradformat file format version followed | e.g. "0.1.0"

siv.dr = 2.5e-6; % required | Bin spacing along axis-1, in the object plane | meters
[siv.defl_ax2, siv.defl_ax1] = gradient(siv.phi, siv.dr); % sic (MATLAB returns first the gradient with respect to the second axis, then the gradient with respect to the first axis)
...
% disp(siv.file_date); % automatically-set | Date the (future) file will be created, in the format "YYYY-MM-DD" | You don't need to set this, it will be set automatically
...
h5filename = fullfile('outs', 'myinversion.h5');
prad_save(siv, h5filename);
